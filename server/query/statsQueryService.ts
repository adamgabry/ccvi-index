...
export async function getStatsSummary(filters?: StatsFilters): Promise<StatsSummaryResponse> {
  ...
  const metricSelect = phase1MapMetrics.map((m) => `
    COUNT("${m}") AS n_${m},
    MIN("${m}") AS min_${m},
  ...
  const metricStats: MetricStats[] = phase1MapMetrics.map((m) => ({
    metric: m,
    n: Number(statsRow[`n_${m}`] ?? 0),
    min: Number(statsRow[`min_${m}`] ?? 0),
    p25: Number(statsRow[`p25_${m}`] ?? 0),
    median: Number(statsRow[`median_${m}`] ?? 0),
    mean: Number(statsRow[`mean_${m}`] ?? 0),
    p75: Number(statsRow[`p75_${m}`] ?? 0),
    max: Number(statsRow[`max_${m}`] ?? 0),
    std: Number(statsRow[`std_${m}`] ?? 0),
  }))
