...
const MARGIN = { top: 20, right: 56, bottom: 96, left: 108 }
const ROW_H  = 68
const DOT_R  = 5
...
export function CountryBeeswarm({ countryScores, metric, geoLabel }: Props) {
  ...
  useEffect(() => {
    ...
    if (!tip) {
      ...
      tip.style.cssText = [
        ...
        'line-height:1.6', 'color:#e2e8f0', 'z-index:9999',
    ...
    const groupKey = (d: CountryScore) =>
      geoLabel && data.length === 1 ? geoLabel : d.continent
    const byContinent = d3.group(data, groupKey)
          ...
          .on('mouseenter', (event: MouseEvent) => {
            ...
            t.innerHTML = `
              <div style="font-weight:700;font-size:13px;margin-bottom:3px">
                ${d.iso3}
                <span style="font-weight:400;color:#94a3b8;font-size:11px"> · ${continent === geoLabel ? geoLabel : continent}</span>
              </div>
              <div>
    ...
    const leg = g.append('g').attr('transform', `translate(0,${innerH + 80})`)
