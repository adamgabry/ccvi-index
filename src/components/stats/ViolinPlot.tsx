...
export function ViolinPlot({ distributions, metric, geoLabel }: Props) {
  ...
  useEffect(() => {
    ...
    const getLabel = (d: ContinentDistribution) =>
      geoLabel && data.length === 1 ? geoLabel : d.continent
    const continents = data.map(getLabel)
    ...
    curves.forEach(({ d, pts }) => {
      ...
      const cy      = (yScale(getLabel(d)) ?? 0) + bw / 2
      const halfVio = bw * 0.46   
      ...
      g.append('rect')
        .attr('x', 0).attr('y', yScale(getLabel(d)) ?? 0)
      ...
      g.append('rect')
        ...
        .on('mousemove', (event: MouseEvent) => {
          ...
          tip.innerHTML = `
            <div style="font-weight:700;margin-bottom:3px">${getLabel(d)}</div>
